...
(function () {
	...
	$(document).ready(function () {
		...
		// alert(fullUrl);
		if (fullUrl) {
			//gets domain from url string ->http://localhost/pdojo/MotherGithub/test_menu13.html?d=myDomain1#
		...
		var loadDefaultAppPromise = FL.API.loadDefaultAppByAPI_Key(API_key).then(function (menuData, homeHTML) {
  ...
  */
		...
		var editPage = function editPage(pageName, htmlStr) {
			...
			tinymce.init({
				...
				relative_urls: false,
				height: 400,
				plugins: ["advlist autolink  link  print preview", "searchreplace wordcount visualchars code fullscreen", "media save contextmenu", "paste textcolor bdesk_photo framelink"],
	...
	exitSaving = function () {
		...
		FL.API.saveHomePage(sHTML).then(function () {
			FL.common.printToConsole("FLpage_editor ->saved ok!");
			FL.API.disconnect().then(function () {
				FL.common.printToConsole("FLpage_editor->after sucessfull saveHomePage --> disconnect ok!");closeWindows();return;
			}, function (err) {
				FL.common.printToConsole("FLpage_editor->after sucessfull saveHomePage -->failure on disconnect err =" + err);return;
			});
		}, function (err) {
			FL.common.printToConsole("FLpage_editor -> failure saving page err=" + err);FL.API.disconnect();
		});
	};
	exitNoSave = function () {
		// alert("exitNoSave");
		...
		FL.API.disconnect().then(function () {
			FL.common.printToConsole("FLpage_editor->no save --> disconnect ok!");closeWindows();return;
		}, function (err) {
			FL.common.printToConsole("FLpage_editor->no save -->failure on disconnect err =" + err);return;
		});
	};
...
//# sourceMappingURL=FLpage_editor-compiled.js.map