var dataLayer = require('./JO2DataLayer');
...
function dir(response){
	...
	exec("dir *.*", function (error, stdout, stderr) {//Non-blocking code to get a list (ls -lah) of all files in current directory (-l:longlisy format, a:include . entries, h:human readable)
		...
		response.writeHead(200, {"Content-Type": "text/plain", 'Access-Control-Allow-Origin' : '*'});//{"Content-Type": "text/javascript"});
...
function fieldGetAll(response,postData,query){
	...
	var entityCN = querystring.parse(query)["entityCN"];
	dataLayer.dlFieldGetAll(entityCN).then(
		function(dictionaryOfFieldsForEntity){
			response.write(JSON.stringify(dictionaryOfFieldsForEntity));
			response.end();
...
function dataGet(response,postData,query){
	...
	dataLayer.dlDataGet(jParam).then(
		function(UIObject){
			response.write(JSON.stringify(UIObject));
			response.end();
			console.log('-----------------------------dataGet------------------------------------------');
		},
		function(error){
...
function dataGetAll(response,postData,query){
	...
	jParam.entityCN = querystring.parse(query)["entityCN"];
	jParam.where = querystring.parse(query)["where"];
	dataLayer.dlDataGetAll(jParam).then(//, getJsonForDataGetAll); // 
		function(listOfRecordsForEntity){
			response.write(JSON.stringify(listOfRecordsForEntity));
			response.end();
...
function dtTableCRUD(response,postData,query){
	...
	var dataArr = querystring.parse(postData)["data[]"];//sent by POST
	var xName = querystring.parse(postData)["data[name]"];//sent by POST
	var xCity = querystring.parse(postData)["data[city]"];//sent by POST
	var xPhone = querystring.parse(postData)["data[phone]"];//sent by POST
	...
	console.log(">>>>Data = "+  xName + "/" + xCity + "/" + xPhone);
	var data = null;
	var dataEdit={
		"id": id,
		"error": "",
	...
	if(action == "create"){
		data = dataEdit;
		console.log("Calling insert with " + JSON.stringify(dataArr));
		//now we call insert 
		data["id"]=270; //the row number
		//we return 
		data["row"] = {//dummy row
			"0": xName,//"NicoCuv",
			"1": xCity,// "Andorre-la-vieille",
			"2": xPhone,// "+40 2123213212",
			"DT_RowId": "row_270"
		};
		console.log("Insert record " + xName + "/" + xCity + "/" + xPhone);
	}else if(action == "edit"){
		data = dataEdit;
		data["id"]=id; //the row number
		data["row"] = {//dummy row
			"0": xName,//"NicoCuv",
			"1": xCity,// "Andorre-la-vieille",
			"2": xPhone,// "+40 2123213212",
		...
		console.log("Edit record " + xName + "/" + xCity + "/" + xPhone);		
	}else if(action == "remove"){
		data = dataEdit;
		data["id"]=-1;
		console.log("Remove record " + JSON.stringify(dataArr));
		//here we need to go to mongo to remove the record 
	}
	var strData = JSON.stringify(data);
	response.writeHead(200, {"Content-Type": "application/json", 'Access-Control-Allow-Origin' : '*'});//{"Content-Type": "text/javascript"});
	response.write(strData);
