...
function dlFieldGetAll(entityCN){
    ...
    try {
		...
		collection.find( {"j.4": entityCN, "j.5":"F"}, {_id:1, "j":1}, function(e,docs) {
			if ( docs != null ) {
				...
				deferred.resolve(formatResponseOK( result ));//callBack (formatResponseOK( result ));
			} else {
				deferred.resolve(formatResponseOK( {} ));//callBack( formatResponseOK({}) );
			}
		});
    }
    catch(err){
		var error = new Error(formatResponseError(99, "dtFieldGetAllByName: " + err));
		console.log(error.message);
		deferred.reject(error);//callBack( formatResponseError(99, "dtFieldGetAllByName: " + err));
    }
...
// data series
function dlDataGet(params){//http://strongloop.com/strongblog/promises-in-node-js-with-q-an-alternative-to-callbacks/
	console.log("Data Layer dlDataGet will be called for entityCN=" + params.entityCN + " and id=" + params.id);
	var deferred = q.defer();
	dlTableDataGet(params).then(
		function(j){ deferred.resolve( formatResponseOK( j ) ); }, function(error){ deferred.reject(error); }
	);
	return deferred.promise;
}
...
// 		callBack(formatResponseOK( j) );//if callback=function(x){console.log(x)}; this would be:function(j){console.log(formatResponseOK( j) )};
// 	});
// }
function dlTableDataGet(params){//Returning a promise from a asynchronous task - http://stackoverflow.com/questions/21999148/not-quite-understanding-the-q-promise-library
	//http://strongloop.com/strongblog/promises-in-node-js-with-q-an-alternative-to-callbacks/
	var deferred = q.defer();//https://www.promisejs.org/ - https://github.com/kriskowal/q/wiki/API-Reference
	...
	try {
		...
		collection.findOne( {_id: id}, {}, function(e, docs){
			...
			if (docs != null){
				deferred.resolve(docs.j);//callBack(docs.j);
			} else{
		...
		var error = new Error("*** !!! Erreur dans dlTabledataGet : " + err + " !!! ***");
		console.log(error.message);
		deferred.reject(error);//callBack({});
	}
	return deferred.promise;
}
// function dlTableDataGet(params, callBack){//http://javascriptissexy.com/understand-javascript-callback-functions-and-use-them/
// 	var entityCN=params.entityCN;
// 	var id=params.id;
// 	console.log("Data Layer dlTableDataGet will be called for entityCN="+entityCN+",id =" + id);
...
// 	}
// }
function dlTableDataGetAll(params){
	var deferred = q.defer();
	...
	try {
		...
		collection.find(where,{},function(e,docs){
			...
			deferred.resolve(j);//callBack(j);
		});
	}
	catch(err)
	{
		var error = new Error("*** !!! Erreur dans dlTableDataGetAll : " + err + " !!! ***");
		console.log(error.message);
		deferred.reject(error);//callBack({});
	}
	return deferred.promise;
...
// 	}
// }
function dlDataGetAll(params){
	console.log("Data Layer dlDataGetAll was called with "+params.entityCN);
	var deferred = q.defer();
	dlTableDataGetAll(params).then(
		function(j){ deferred.resolve( formatResponseOK( j ) ); }, function(error){ deferred.reject(error); }
	);
	return deferred.promise;
}
...
function mountHeaderJson(listOfTableColumns,listOfDictionaryFields){
	var header = [];
...
function mountRowsJson(header,listOfTableColumns,listOfRecords){
	var data = [];
	for (var recordIndex=0; recordIndex < listOfRecords.length; recordIndex++ ) {
		var record = {};
		...
		for (var i=0; i < listOfTableColumns.length; i++ ){//monting a single record only with fields belonging to listOfTableColumns
			...
			var fieldValue = listOfRecords[recordIndex][fieldCN];
			record[fieldName] = fieldValue;
			// console.log("record "+ recordIndex + " fieldCN=" + fieldCN + " fieldName=" + fieldName + " fieldValue=" + fieldValue);
		}
		record["DT_RowId"] = "row_"+recordIndex;
		data.push(record); //record is complete and pushed into the data array
		// console.log("record "+ recordIndex + " =" + JSON.stringify(data[recordIndex]) + " was mounted.");
	}
	return data;
}
function dlHalfDummyGrid(entityCN,callBack){
	...
	var gridData = null;
	dlDataGet({entityCN:"1",id:"custBrowse"}).then(
		function(UIObject){
			var tableCN = UIObject.j["02"];//the base table of the form
			...
			// -------------------------------- header block ------------------------------------------------------------------
			dlFieldGetAll(tableCN).then(//,getDictionaryFieldsForUIObject);//ex localhost:3000/fieldGetAll?entityCN=00
				function(dictionaryFieldsForUIObject){
					var listOfDictionaryFields = dictionaryFieldsForUIObject.j;//array of Dictionary Fields
					var header = mountHeaderJson(listOfTableColumns,listOfDictionaryFields);
					console.log("beginning of data block");
					// -------------------------------- data block ------------------------------------------------------------------
					dlDataGetAll({entityCN:tableCN,where:null}).then(//ex dataGetAll?entityCN=00
						function(jsonOfRecords){
							var listOfRecords = jsonOfRecords.j;
							console.log("returned form dlDataGetAll with "+ listOfRecords.length + " Records ! ");
							var data = mountRowsJson(header,listOfTableColumns,listOfRecords);
...
// function OlddlHalfDummyGrid(entityCN,callBack){
// 	console.log("Data Layer dlHalfDummyGrid will be called for entityCN="+entityCN);
// 	var gridData = null;
...
// 				callBack(gridData);
// 			};
// 			console.log("Will call dlDataGetAll for entityCN="+ tableCN);
// 			dlDataGetAll({entityCN:tableCN,where:null},getAllRecordsFromEntityCN);//ex dataGetAll?entityCN=00
// 		};
