...
FL["links"] = (function () {
	...
	var XgetMailchimpHTML = function XgetMailchimpHTML(cId) {
		var def = $.Deferred();
		var arr = null;
		var fl = FL.fl;
		if (fl) {
	...
	var XconvertsToArrOfObj = function XconvertsToArrOfObj(templateOptionsArr) {
	...
	var mountTemplate = function mountTemplate(jsonObj, header, body, footer) {
	...
	var set3ButtonsAndGrid = function set3ButtonsAndGrid(entityName) {
		//displays addGrid, delete and editGrid buttons (with clicks prepared) and services to the right
		$("#_editGrid").off("click");
	...
	var DefaultGridWithNewsLetterAndEditButtons = function DefaultGridWithNewsLetterAndEditButtons(entityName) {
		...
		if (FL.dd.isHistoMailPeer(entityName)) {
			...
			promise = FL.API.createTableHistoMails_ifNotExisting(entityName).then(function () {
				// this.setSync(FL.dd.histoMailPeer(entityName),true);
				// set3ButtonsAndGrid(entityName);
				FL.grid.displayDefaultGrid(entityName); //loads from server and display buttons and Grid
				return;
			}, function (err) {
				alert("FL.links.DefaultGridWithNewsLetterAndEditButtons ERROR: cannot create histoMail peer for " + entityName + " - " + err);return;
			});
			// set3ButtonsAndGrid(entityName);//displays addGrid, newletter and editGrid buttons (with clicks prepared) and displays grid
		}
	};
	var DefaultGridWithNewsLetterAndEditButtonsByCN = function DefaultGridWithNewsLetterAndEditButtonsByCN(eCN) {
		//TO BE COMPLETED
		//A)shows add button, newsletter and grid edit buttons if an email field exist in entityName
		//  	checks if _histoMail_<ecn(entityName)> exists. If not creates it.
		//B)shows add button and grid edit buttons if no email field exist in entityName
	...
	return {
		...
		test: function test(x) {
			//call with menu key "uri": "javascript:FL.links.test('JOJO')"
			internalTest(x);
		...
		appSettings: function appSettings() {
			...
			var options = {
				...
				dropdown: {
					...
					"_editAppSettings_currency_options": {
						arr: [{ value: 1, text: "$ (Dollar)", op: "$" }, { value: 2, text: "€ (Euro)", op: "€" }, { value: 3, text: "Kr (Krona)", op: "Kr " }, { value: 4, text: "Kz (Kwanza)", op: "Kz " }, { value: 5, text: "MT (Metical)", op: "MT " }, { value: 6, text: "R$ (Real)", op: "R$ " }, { value: 7, text: "元 (Renmimbi)", op: "元" }, { value: 7, text: "₹ (Rupee)", op: "₹" }, { value: 8, text: "Rs (Rupee)", op: "Rs " }, { value: 8, text: "¥ (Yen)", op: "¥" }],
			...
			FL.common.editMasterDetail("B", " Edit application settings", "_editAppSettingsTemplate", masterDetailItems, options, function (result) {
				if (result) {
					...
					if (thousandsSeparator === "") FL.common.makeModalInfo("Empty information => no changes in application setting");else {
		...
		appSettingsForceButton: function appSettingsForceButton() {
		...
		pageEditor: function pageEditor(xPage) {
			//call with menu key "uri": "javascript:FL.links.pageEditor('home')"
			...
			function checkChild() {
				if (child.closed) {
					// we need this to show the new home page
					clearInterval(timer);
					FL.login.home();
				} else {}
			}
		},
		newsletterEditor: function newsletterEditor() {
			//call with menu key "uri": "javascript:FL.links.newsletterEditor()"
		...
		setDefaultGridByCN: function setDefaultGridByCN(eCN) {
			//called with menu key "uri": "javascript:FL.links.setDefaultGridByCN('55')"
		...
		setDefaultGridByCN2: function setDefaultGridByCN2(eCN) {
			//called with menu key "uri": "javascript:FL.links.setDefaultGridByCN('55')"
			if (FL.dd.isEntityInLocalDictionaryByCN(eCN)) {
				if (FL.dd.isEntityByCNInSync(eCN)) {
					//eCN exists in local dictionary and is in sync
					DefaultGridWithNewsLetterAndEditButtons(entityName);
				} else {
					//entityName exists but is not in sync - we force synchronization
					alert("FL.links.setDefaultGrid - " + entityName + " not in sync we will syncronize local to backend.");
					FL.API.syncLocalDictionaryToServer(entityName).then(function () {
						DefaultGridWithNewsLetterAndEditButtons(entityName);return;
					}, function (err) {
						alert("FL.links.setDefaultGrid ERROR: cannot sync " + entityName + " to server!");return;
					});
				}
			} else {
				//entity is not in local dictionary =>we force an update of local dictionary with server dictionary data
		...
		setDefaultGrid: function setDefaultGrid(entityName) {
			//called with menu key "uri": "javascript:FL.links.setDefaultGrid('JOJO')"
			...
			if (FL.dd.isEntityInLocalDictionary(entityName)) {
				if (FL.dd.isEntityInSync(entityName)) {
					//entityName exists in local dictionary and is in sync
					DefaultGridWithNewsLetterAndEditButtons(entityName);
				} else {
					//entityName exists but is not in sync - we force synchronization
					alert("FL.links.setDefaultGrid - " + entityName + " not in sync we will syncronize local to backend.");
		...
		clearDictionary: function clearDictionary() {
		...
		userGrid: function userGrid() {
			//call with menu key "uri": "javascript:FL.links.setDefaultGrid('JOJO')"
			FL.common.makeModalInfo("To be implemented.");
		},
		userAdministration: function userAdministration() {
			//call with menu key "uri": "javascript:FL.links.setDefaultGrid('JOJO')"
		...
		editStylesAndFonts: function editStylesAndFonts() {
			//call with menu key "uri": "javascript:FL.links.setDefaultGrid('JOJO')"
		...
		resetMenus: function resetMenus() {
			//saves factory default menu in current user"
		...
		getMandrillRejectListForSender: function getMandrillRejectListForSender(senderEmail) {
			//returns Madrill rejectList for sender=senderEmail
			...
			promise.fail(function (err) {
				FL.common.printToConsole(">>>>> FL.links.getMandrillRejectListForSender() FAILURE returning list<<<<<" + err);return def.reject(err);
			});
		},
		setMandrillDeleteFromReject: function setMandrillDeleteFromReject(arrayOfEmails) {
			//delete Emails from  Madrill rejectList
		...
		displayStatistics: function displayStatistics() {
			//show all user statistics in mandrill
			var user = "test";
			...
			promise.done(function (data) {
				...
				$("#_placeHolder").append("<dl style=\"margin-left: 30px\"></dl>");
				_.each(data, function (element) {
					$("#_placeHolder dl").append("<dt><span class=\"tab\" >" + element.address + " - created at " + element.created_at + "</span></dt>" + "<dd style=\"margin-left: 30px\"> Sent         :" + element.sent + "</dd>" + "<dd style=\"margin-left: 30px\"> Hard-bounces :" + element.hard_bounces + "</dd>" + "<dd style=\"margin-left: 30px\"> Soft-bounces :" + element.soft_bounces + "</dd>" + "<dd style=\"margin-left: 30px\"> Rejects      :" + element.rejects + "</dd>" + "<dd style=\"margin-left: 30px\"> Complaints    :" + element.complaints + "</dd>" + "<dd style=\"margin-left: 30px\"> Unsubs       :" + element.unsubs + "</dd>" + "<dd style=\"margin-left: 30px\"> Opens        :" + element.opens + "</dd>" + "<dd style=\"margin-left: 30px\"> Clicks       :" + element.clicks + "</dd>" + "<dd style=\"margin-left: 30px\"> Unique-opens :" + element.unique_opens + "</dd>" + "<dd style=\"margin-left: 30px\"> Unique-clicks:" + element.unique_clicks + "</dd>");
				});
				return;
			});
			promise.fail(function (err) {
...
//# sourceMappingURL=FLmenulinks2-compiled.js.map