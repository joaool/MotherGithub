 ...
 */
	...
	FL['login'] = (function () {
		...
		var stylesForSelection = [{ value: 0, text: 'cerulean' }, { value: 1, text: 'cosmos' }, { value: 2, text: 'readable' }, { value: 3, text: 'red' }, { value: 4, text: 'spacelab' }];
		var fontFamilyForSelection = [{ value: 0, text: 'helvetica' }, { value: 1, text: 'menlo' }, { value: 2, text: 'georgia' }, { value: 3, text: 'bookantiqua' }, { value: 4, text: 'lucida' }, { value: 5, text: 'tahoma' }, { value: 6, text: 'elite' }];
		var messageEnabler = (function () {
			//a singleton (to be instantiated by checkSignIn) to receive and dispatch topic messages.http://addyosmani.com/resources/essentialjsdesignpatterns/book/#singletonpatternjavascript
			var instantiated;
			function init() {
				var lastMessage = null;
				var receiver = function receiver(triggerName, enter) {
					//enter true means ENTERING the slide panel trigger, false means EXITING
					...
					if (triggerName == '#trigger1') {
						//user entered or exited slide panel fl_settings.html
						...
						//user entered or exited slide panel fl_builder.html
		...
		var recoverLastMenu = function recoverLastMenu() {
			//recover locally saved menu and informs FL.menu about the new menu if any
			var lastMenuStr = localStorage.getItem('storedMenu');
			var lastMenuObject = null;
			if (lastMenuStr) lastMenuObject = JSON.parse(lastMenuStr);
			if (lastMenuObject) {
		...
		var displaySignInUser = function displaySignInUser(user) {
			//if user is null displays signIn icon+"Sign In" otherwise displays user
			var htmlToInject = '<div style="line-height:2.2em;"><span id="_signInDomain" class="small hidden-xs" style="margin-left:6.5em" >' + 'Welcome to FrameLink: support@framelink.co App:' + FL.domain + '</span>' +
			// '<a class="pull-right text-muted" href="javascript:FL.signIn()" style="margin-right:12em">'+
			'<a class="pull-right" href="javascript:FL.login.signIn()" style="margin-right:12em;">' + '<img src="FL_ui/img/signIn.png">' + '<span id="_signIn" style="font-size: 1.2em;"> Sign In</span>' + '</a>' +
			...
			'</div>';
			if (user) {
				htmlToInject = '<div style="line-height:2.2em;"><span class="small hidden-xs" style="margin-left:6.5em">' + 'Welcome to FrameLink: support@framelink.co App:' + FL.domain + '</span>' +
				// '<a class="pull-right text-muted" href="javascript:FL.signIn()" style="margin-right:12em">'+
				'<a class="pull-right " href="javascript:FL.login.signIn()" style="margin-right:12em">' + '<i  class="glyphicon glyphicon-user"></i>' + '<span id="_signIn" style="font-size: 1.2em;"> ' + user + '</span>' + '</a>' +
				// '<a class="pull-right text-muted" style="margin-right:1.5em" href="javascript:FL.tourIn()" >'+
				// '<a class="pull-right" style="margin-right:1.5em;" href="javascript:FL.tourIn()" >'+
				// 	'<i class="glyphicon glyphicon-eye-open" style="font-size: 1.5em;color:black;"></i>'+
				// 	'<span style="font-size: 1.2em;"> Tour</span>'+
		...
		var logOutMenu = function logOutMenu() {
			//displays the menu and home page for logout status
		...
		var loginDefaultMenu_and_homePage = function loginDefaultMenu_and_homePage(loginObject) {
			//after user creation in server we need to display the menu and home page for logins of new users (defaults for menu and homepage)
		...
		var appSetup = function appSetup(loginObject) {
			//(local dict + menu + style + fontFamily) from server, and displays UI accordingly ->menu, style, font and homepage
		...
		var loginDialogs = function loginDialogs(loginObject) {
			var def = $.Deferred();
			...
			if (FL.fl) btn1Caption = 'Logout'; //if it enters in loggedin mode the first button is "Logout"
			FL.common.editMasterDetail('B', ' Sign in to FrameLink', 'loginTemplate', masterDetailItems, { type: 'primary', icon: 'user', button1: btn1Caption, button2: 'Sign in' }, function (result) {
				if (result) {
					...
					loginPromise.then(function (userExists, loginObject) {
						...
						if (!userExists) {
							FL.common.makeModalInfo('Invalid Email/password.<br> - If you are an existing user please correct your EMail/password.<br>' + ' - If you are a <strong>new user</strong> please press <strong>\'Logout\'</strong> and then ' + '<strong>\'Get started\'</strong> button.<br>', function () {
					...
					//the user choosed Cancel or Logout (it is the same button)
		...
		var getStartedDialogs = function getStartedDialogs(loginObject) {
			var def = $.Deferred();
			var masterDetailItems = {
				master: { eMail: loginObject.eMail, password: loginObject.password },
				detail: {}
			...
			FL.common.editMasterDetail('B', ' Get started with FrameLink', 'getStartedTemplate', masterDetailItems, { type: 'primary', icon: 'thumbs-up', button1: 'Cancel', button2: 'Start now' }, function (result) {
				if (result) {
					...
					if (email.length > 0 && password.length === 0) {
						...
						newUserPromise.fail(function (err) {
							alert('getStartedDialogs --> error in newUserAccess for ' + loginObject.email + ' !!!!');
						});

						def.resolve(true); //true ==>continue false=>repeat	
					}
				} else {
					//the user choosed logout
		...
		var loginAccess = function loginAccess(loginObject) {
			//when login email is valid and other data acceptable - {email:email,userName:userName,password:password};
			...
			if (loginObject.email == 'guest@framelink.co') {
				...
				loginPromise = FL.API.prepareTrialApp().then(function () {
					return appSetup(loginObject);
				})
				...
				//use an existing account or a new account
				...
				existingUserPromise.done(function (exist) {
					...
					if (exist) {
						...
						var setupExistingUserPromise = FL.API.connectUserToDefaultApp(loginObject.email, loginObject.password).then(function () {
							return appSetup(loginObject);
						}).then(function () {
		...
		var newUserAccess = function newUserAccess(loginObject) {
			//when login email is valid and other data acceptable - {email:email,userName:userName,password:password};
			// loginObject = {email:email,userName:userName,password:password};
			var def = $.Deferred();
			var loginPromise = null;
			...
			existingUserPromise.done(function (exist) {
				...
				if (!exist) {
					//the user does not exist we can create-it
					...
					createUserPromise = FL.API.createUserAndDefaultApp(loginObject.email, loginObject.password, 'First app of ' + userName).then(function () {
						...
						FL.common.printToConsole('FLLoadCss2 ->newUserAccess->fail');return def.reject(err);
					});
				} else {
					//user exist !!!
		...
		var recoverLastTourActiveStatus = function recoverLastTourActiveStatus() {
			//recover locally saved menu and informs FL.menu about the new menu if any
		...
		var resetStyle = function resetStyle(is_logIn) {
			//if is_login = true display last locally saved style else display FLreadable.css
			...
			if (is_logIn) {
				//a soft reset - the content from local storage will be used. Otherwise force default values
				if (lastStyleStr) currentStyle = lastStyleStr;
				if (lastFontFamilyStr) currentFontFamily = lastFontFamilyStr;
			}
		...
		return {
			...
			defaultPage: '<div class=\'jumbotron\'>' + '<h1>Welcome</h1>' + '<p>Hello <strong><%= shortUserName %></strong>, your are logged into FrameLink !</p>' + '<p>Application description: <%= appDescription %></p>' + '</div>',
			defaultPageOnLogout: '<div class=\'jumbotron\'>' + '<h1>FrameLink Platform</h1>' + '<p>Please <strong>Sign In</strong> as a designer (upper right corner) or choose ' + '<strong>Get started</strong> if you are a first time user.</p>' + '<p>' +
			// "<a href='#' class='btn btn-primary btn-large' onclick='FL.showTourStep0 = true; FL.tourIn();'>Tour</a>" +
			'<a href=\'#\' class=\'btn btn-primary btn-large\' onclick=\'FL.login.getStarted();\'>Get started with a FREE beta account</a>' + '</p>' + '</div>',
			...
			signOut: function signOut() {
				//saves logout in local storage ->hide slide panels  ->updates upper right corner display
			...
			checkSignIn2: function checkSignIn2() {
			...
			signIn: function signIn() {
				//called from href in first line
				...
				if (FL.is_sidePanelOpen) {
					...
					if (typeof Storage !== undefined) {
						...
						if (lastLoginStr == 'undefined') lastLoginStr = undefined;
						if (lastLoginStr) {
			...
			getStarted: function getStarted(loginObject) {
			...
			clearAllSettings: function clearAllSettings() {
				//restore original menu in local storage and updates DOM status - called directly from DOM link
...
//user entered or exited slide panel fl_service.html
// alert("FLLoadCss.js init.receiver() -->Entered trigger3");

//# sourceMappingURL=FLLoadCss2-compiled.js.map