 ...
 */
	...
	FL['login'] = (function () {
		...
		var displaySignInUser = function displaySignInUser(user) {
			...
			// 					'</a>'+
			// 				'</div>';
			var htmlToInject = '<span id="_signInDomain" class="small" style="margin-left:2%;line-height:2.2em; display:inline;float:left" >' + 'Welcome to FrameLink: support@framelink.co App:' + FL.domain + ' / ' + browserW + '</span>' + '<a class="pull-right" href="javascript:FL.login.signIn()" style="margin-right:2%;line-height:2.2em; display:inline;float:left">' + '<img src="FL_ui/img/signIn.png">' + '<span id="_signIn" style="font-size: 1.2em;"> Sign In</span>' + '</a>';
			if (user) {
				...
				// 					'</a>'+
				// 				'</div>';
				htmlToInject = '<span class="small" style="margin-left:2%;line-height:2.2em; display:inline;float:left">' + 'Welcome to FrameLink:' + FL.domain + ' / ' + browserW + '</span>' +
				// '<a class="pull-right text-muted" href="javascript:FL.signIn()" style="margin-right:12em">'+
				'<a class="pull-right " href="javascript:FL.login.signIn()" style="margin-right:2%;line-height:2.2em; display:inline;float:left">' + '<i  class="glyphicon glyphicon-user"></i>' + '<span id="_signIn" style="font-size: 1.2em;"> ' + user + '</span>' + '</a>';
			}
		...
		var getLoginObject = function getLoginObject() {
		...
		var setupApp = function setupApp(menuData, homeHTML, loginObject) {
		...
		var loginDialogs = function loginDialogs(loginObject) {
			...
			if (FL.fl.getsId()) btn1Caption = 'Logout'; //if it enters in loggedin mode the first button is "Logout"
			...
			existingUserPromise.done(function (exist) {
				...
				if (!exist) {
					...
					createUserPromise = FL.API.createUserAndDefaultApp(loginObject.email, loginObject.password, 'First app of ' + userName).then(function () {
						...
						FL.common.printToConsole('FLlogin2newUserAccess->fail');return def.reject(err);
					});
				} else {
					//user exist !!!
					spinner.stop();
		...
		return {
			...
			signIn: function signIn() {
				//called from href in first line of screen (htmlToInject)
...
//# sourceMappingURL=FLlogin2-compiled.js.map