...
FL["dd"] = (function () {
	...
	//     
	var getCompressed = function getCompressed(iGenNext) {
		//returns a 2 bytes string from number
		var sOut = "";
		//-------------
		var iMinLen = 2;
	...
	var strReverse = function strReverse(str) {
	...
	var _preArticle = function _preArticle(sWord, xLanguage) {
		//for language xLanguage extract the article to be used before sWord
		...
		switch (xLanguage) {
			...
			case "Fr":
				...
				if ("ée ie té".indexOf(xLast2) >= 0) xArticle = "une";
				var xLast3 = sWord.substr(xLen - 3, 3);
				if ("ade eur rie ère ine ise ose ole ure".indexOf(xLast3) >= 0) xArticle = "une";
				var xLast4 = sWord.substr(xLen - 4, 4);
				if ("aille ison sion tion ance asse étée ence euse elle esse ette".indexOf(xLast3) >= 0) xArticle = "une";
				break;
			case "Pt":
				if (xLast == "a") xArticle = "uma";
				break;
		}
		return xArticle + " " + sWord;
	}; //preArticle			
	var _plural = function _plural(sWord, xLanguage) {
		//returns the plural of sWord in language=xLanguage
		...
		switch (xLanguage) {
			case "En":
				// var xLast2=sWord.substr(xLen-2,2);
				if ("x".indexOf(xLast) >= 0) {
					//if last char is one of
					xWord = xWord + "es"; //box->boxes
				} else if ("y".indexOf(xLast) >= 0) {
					//if last chars are one of
					xWord = xWord.substr(0, xLen - 1) + "ies"; //city ->cities
				} else if ("ch to ro".indexOf(xLast2) >= 0) {
					//if last 2 chars are one of
					...
					if ("disco piano photo".indexOf(sWord) >= 0) //exceptions to the above rule
						xWord = sWord + "s";
				} else if ("fe".indexOf(xLast2) >= 0) {
					//if last 2 chars are one of
					xWord = xWord.substr(0, xLen - 2) + "ves"; //wife ->wives
				} else if ("lf".indexOf(xLast2) >= 0) {
					//if last 2 chars are one of
			...
			case "Fr":
				...
				if ("xs".indexOf(xLast) >= 0) {
					//if last char is one of
					xWord = xWord; //prix->prix gros->gros
				} else if ("al".indexOf(xLast2) >= 0) {
					//if last 2 chars are one of
					xWord = xWord.substr(0, xLen - 1) + "ux"; //jornal -> jornaux
				} else if ("au eu".indexOf(xLast2) >= 0) {
					//if last 2 chars are one of
	...
	var makeSemStr = function makeSemStr(x1, x2, xType, xLanguage) {
		//concatenate basic entity semantics
	...
	var entitySemantics = function entitySemantics(oEntity, xType, xLanguage) {
		//shows entity/description semantics
	...
	var attributeSemantics = function attributeSemantics(xAttribute, xAttrDescription, oEntity, xLanguage) {
		//shows singular-description for attribute=xAttribute
	...
	var reversePhrase_is_something = function reversePhrase_is_something(sVerb) {
	...
	var invertedToOneVerb = function invertedToOneVerb(sVerb, xLanguage) {
		//returns verb of inverted relation of sVerb
		...
		switch (xLanguage) {
			case "En":
				if (xArr.length > 1) {
					//more than one word
					if (xArr.length > 3) {
						if (xArr[1] == "responsable" && xArr[2] == "by") {
							//"it begins by is responsable by" Ex:"is responsable by complaints"
							xRet = xArr[3] + " to"; //returns "complaints to"
						} else {
							xRet = reversePhrase_is_something(sVerb);
						}
					} else if (xArr[0] == "is") {
						//"is published by" ->publishes <> "is managed by" ->manages
						xRet = reversePhrase_is_something(sVerb);
					}
				} else if (xArr.length == 1) {
					//one single word -> "publishes" ->"is published by", "visits"->"is visited by"
					xLen = xArr[0].length;
			...
			case "Pt":
				if (xArr.length > 1) {
					//more than one word
					if (xArr[0] == "é") {
						//"é publicado por " ->publica
						xLen = xArr[1].length;
						var xLast3 = xArr[1].substr(xLen - 3, 3); //last 2 chars (ed)
						if (xLast3 == "ado") xRet = xArr[1].substr(0, xLen - 3) + "a";
					}
				} else if (xArr.length == 1) {
					//one single word -> "publica" ->"é publicado por", "visita"->"é visitado por"
					xLen = xArr[0].length;
					xLast = xArr[0].substr(xLen - 1, 1); //last 1 chars (s)
					xLast2 = xArr[0].substr(xLen - 2, 2); //last 2 chars (s)
					if (xLast2 == "xx") {
						...
						//"visita"->"is visited by"
						xRet = "é " + xArr[0] + "do por";
					}
				}
				break;
		} //end switch
		if (!xRet) xRet = oObj["default"];
		return xRet;
	}; //invertedToOneVerb			
	var cardinalityDecoder = function cardinalityDecoder(sCardinality, xLanguage) {
		//returns semantics to Cardinality - object {cardText:<string or null>,cardPlural:<boolean>}
		...
		xCardPlural = false;
		if (oLanguage) {
			//the quest is for a known language
	...
	var _relationSemantics = function _relationSemantics(sSingular, sRightEntity, sVerb, sCardinality, xLanguage) {
		//shows relationsemantics
	...
	var attributeIndex = function attributeIndex(xSingular, xAttribute) {
		//for entity=xSingular, returns the index of attribute=xAttribute
	...
	var prepareRelation = function prepareRelation(xSingular, rCN, withEntityName, verb, cardinality, side, storedHere, xLanguage) {
		//prepare JSON to relation arr - used by FL.dd.addRelation()
		var relation = {};
		relation["rCN"] = rCN;
	...
	var getDictAttributesBackup = function getDictAttributesBackup(attrArr) {
		//returns a copy of an attributes Array (to be used by getDictEntityBackup)
	...
	return {
		...
		preArticle: function preArticle(sWord, xLanguage) {
			//for language xLanguage extract the article to be used before sWord
			//Ex z="What is "+dDictionary.preArticle(xEntity,"En")+"?";
			return _preArticle(sWord, xLanguage);
		},
		plural: function plural(sWord, xLanguage) {
			//returns the plural of sWord in language=xLanguage				
			return _plural(sWord, xLanguage);
		},
		clear: function clear() {
			this.entities = { __Last: 9999, __LastRelation: 0 };
		},
		relationSemantics: function relationSemantics(sSingular, sRightEntity, sVerb, sCardinality, xLanguage) {
			return _relationSemantics(sSingular, sRightEntity, sVerb, sCardinality, xLanguage);
		},
		displayEntities: function displayEntities() {
			//display all entities with their attributes and relations
			...
			for (var key in oEntities) {
				if (oEntities.hasOwnProperty(key)) {
					//this restrain the iteration only to the object's own attributes
					if (key == "__Last") {
						...
						if (xArr.length > 0) {
							...
							for (var i = 0; i < xArr.length; i++) {
								...
								if (FL.common.typeOf(xSpecialTypeDef) == "object") xSpecialTypeDef = JSON.stringify(xSpecialTypeDef);
								var xKey = xArr[i].key;
		...
		countEntitiesBeginningBy: function countEntitiesBeginningBy(singularPrefix) {
			//return the number of entities whose singular name begins by singularPrefix
		...
		nextEntityBeginningBy: function nextEntityBeginningBy(singularPrefix) {
			//proposes a name
		...
		createEntity: function createEntity(xSingular, xDescription) {
			//add an entity entry returning true if it succeeds false otherwise
			...
			var xPlural = null;
			var xRet = false;
			if (!this.entities[xSingular]) {
				//xSingular does not exist in dictionary
				...
				xPlural = _plural(xSingular, "En"); //+"s";
		...
		updateEntityBySingular: function updateEntityBySingular(xSingular, xOptions) {
			//updates an existing entity. It can uodate the entity key "singular"
			...
			var xRet = false;
			if (this.entities[xSingular]) {
				//xSingular exists in dictionary
				...
				entityUpdatePromise.done(function (result) {
					FL.common.printToConsole(">>>>> updateEntityBySingular SUCCESS count=" + result + "<<<<<");return def.resolve(result);
				});
				entityUpdatePromise.fail(function (err) {
					FL.common.printToConsole(">>>>> updateEntityBySingular FAILURE <<<<< " + err);return def.reject(err);
				});
				//  oEntity.sync = false;
				// xRet = true;
			} else {
				//xSingular does not exists in dictionary
		...
		updateEntityByCName: function updateEntityByCName(xCName, xOptions) {
			//updates entity with compressed name = xCName with xOptions object
			...
			if (oEntity) {
				...
				//xCName does not exists in dictionary
		...
		getCEntity: function getCEntity(xEntity) {
			//returns the compressed name of a logical name xEntity - if does not exist =>returns null
			//returns the compressed name of the logical name xEntity. The method is a NOP if xEntity is null;
			var xCEntity = null;
			if (xEntity) {
				var oEntities = this.entities;
				if (oEntities[xEntity]) xCEntity = oEntities[xEntity].csingular;
			}
			return xCEntity;
		},
		getEntityByCName: function getEntityByCName(eCN) {
			//returns the  logical name for compressed name = eCN. If not found returns null
			...
			var oEntity = _.find(valuesArr, function (element) {
				//element is each array element. If is an object it returns the value
			...
			if (oEntity) entityName = oEntity.singular;
			return entityName;
		},
		addAttribute: function addAttribute(xSingular, xAttribute, xDescription, xLabel, xType, xTypeUI, arrEnumerable) {
			//adds AttributeName,Description, label Type and enumerable to entity = xSingular			
			...
			if (oEntity) {
				...
				if (xIndex < 0) {
					...
					//({name:"address",description:"address to send invoices",label:"Address",type:"string",enumerable:null,key:false});
					if (xType != "string" && xTypeUI != "combobox") arrEnumerable = null;
					...
					//updates the existing attribute - if xKey is true (it will continue to be true)
		...
		upsertAttribute: function upsertAttribute(xSingular, xAttribute, options) {
			// upserts attribute in table with logical name = sSingular
			// xSingular - Entity name (singular) to add attribute
			// xAttribute - attribute name (Nico's "3")
			...
			// enforcements -------
			if (propsToUse.type != "string" && propsToUse.typeUI != "combobox") propsToUse.enumerable == null;
			if (propsToUse.typeUI != "textbox" && propsToUse.typeUI != "integerbox" && propsToUse.typeUI != "numberbox" && propsToUse.typeUI != "percentbox" && propsToUse.typeUI != "currencybox") propsToUse.mask == null;
			if (propsToUse.typeUI != "lookupbox") propsToUse.specialTypeDef == null;
			// -------------------
			var oEntity = this.entities[xSingular];
			if (oEntity) {
				var xIndex = attributeIndex(xSingular, xAttribute);
		...
		getEntityAttribute: function getEntityAttribute(xSingular, xAttribute) {
		...
		updateAttribute: function updateAttribute(xSingular, xAttribute, changeObj) {
			...
			if (oEntity) {
				...
				if (xIndex >= 0) {
					...
					if (oAttributes.name != xAttribute) {
						//attribute name was changed. It is necessary to update L2C and C2L
		...
		addRelation: function addRelation(xSingular, withEntityName, verb, cardinality, side, storedHere, xLanguage) {
			//adds a new relation to the array of relations of entity xSingular
			...
			if (oEntity) {
				if (oToEntity) {
					...
					if (FL.dd.isRelation(xSingular, rCN)) {
						...
						if (cardinality == "1") cardinality2 = "N";
		...
		relationPass2: function relationPass2() {
			...
			var valuesArr = _.values(this.entities);
			//note for _.find: if valuesArr is and array of objects =>element is the value of each key/value and returns value
			_.each(valuesArr, function (element) {
				//element is each array element. If is an object it returns the value
				if (typeof element != "number") {
					// FL.common.printToConsole("Pass2 -->"+element.singular);
					var xSingular = element.singular;
					_.each(element.relations, function (relation) {
						//relation is a relation from the current entity
		...
		isRelation: function isRelation(xSingular, rCN) {
			...
			if (oEntity) {
				...
				if (arrPos >= 0) exists = true;
			}
			return exists;
		},
		getEntityBySingular: function getEntityBySingular(xSingular) {
			//returns an entity and its attributes
			var oEntity = this.entities[xSingular];
			return oEntity;
		},
		isEntityInLocalDictionary: function isEntityInLocalDictionary(entityName) {
			//returns true if entityName exists in local dictionary
			var exists = false;
			if (this.getEntityBySingular(entityName)) //if getEntityBySingular is not null =>exists = true
				exists = true;
			return exists;
		},
		isEntityInLocalDictionaryByCN: function isEntityInLocalDictionaryByCN(eCN) {
			//returns true if compressed entity name (eCN) exists in local dictionary
			var exists = false;
			if (this.getEntityByCName(eCN)) exists = true;
			return exists;
		},
		histoMailPeer: function histoMailPeer(entityName) {
			var eCN = this.getCEntity(entityName);
			return histoName = "_histoMail_" + eCN;
		},
		peerTypeFor: function peerTypeFor(typeOfPeer, entityName) {
			//for type histoMail and entity ecn=55 forms "_histomail_55"
			var eCN = this.getCEntity(entityName);
			return histoName = "_" + typeOfPeer + "_" + eCN;
		},
		isHistoMailPeer: function isHistoMailPeer(entityName) {
			//returns true if _histoMail_<ecn(entityName)> exists in local dictionary
			var exists = false;
		...
		isHistoMailPeerByCN: function isHistoMailPeerByCN(ecN) {
			//returns true if _histoMail_<ecn> exists in local dictionary
			var exists = false;
		...
		isEntityWithTypeUI: function isEntityWithTypeUI(entityName, typeUI) {
			//returns true if entityName has an Email field
			var exists = false;
			var oEntity = this.entities[entityName];
			if (oEntity) {
				//searches for the first Email field
		...
		firstEmailAttribute: function firstEmailAttribute(eCN) {
			var emailAttr = null;
			...
			if (!_.isUndefined(el)) emailAttr = el.name;
			return emailAttr;
		},
		isEntityInSync: function isEntityInSync(entityName) {
			//returns true if entityName has in sync status = true
			var exists = false;
			var oEntity = this.entities[entityName];
			if (oEntity) {
				return oEntity.sync;
		...
		isEntityByCNInSync: function isEntityByCNInSync(eCN) {
			//returns true if eCN has in sync status = true - if eCN does not exist returns false
			var entityName = this.getEntityByCName(eCN);
			if (entityName) {
				var oEntity = this.entities[entityName];
				if (oEntity) {
		...
		setSync: function setSync(xSingular, bStatus) {
			//set sync = true for entity= xSingular - nothing is done if entity does not exist
			var oEntity = this.entities[xSingular];
			if (oEntity) {
				oEntity.sync = bStatus;
			}
		},
		setFieldTypeUI: function setFieldTypeUI(xSingular, fieldName, newTypeUI) {
			//sets typeUI = newTypeUI for field = fieldName in entity xSingular
			var oEntity = this.entities[xSingular];
			if (oEntity) {
				oEntity.sync = false;
				var el = _.find(oEntity.attributes, function (element) {
		...
		createHistoMailPeer: function createHistoMailPeer(entityName) {
			//create histoMail peer in local dict
		...
		removeHistoMailPeer: function removeHistoMailPeer(entityName) {
			//remove histoMail peer in local dict
			var fName = this.histoMailPeer(entityName);
			if (this.isEntityInLocalDictionary(fName)) {
		...
		setFieldCompressedName: function setFieldCompressedName(xSingular, fieldName, fieldCN) {
			//for entity xSingular and attribute fieldName sets compressed name
			var oEntity = this.entities[xSingular];
			if (oEntity) {
				oEntity.sync = false;
		...
		getFieldCompressedName: function getFieldCompressedName(xSingular, fieldName) {
			//for entity xSingular and attribute fieldName gets compressed name
			var oEntity = this.entities[xSingular];
			var fieldCN = null;
			if (oEntity) {
		...
		getArrayOfFields: function getArrayOfFields(xSingular) {
			//for entity xSingular return an array of JSON including statment
		...
		setEntityFieldsBySingular: function setEntityFieldsBySingular(xSingular, fieldsArr, changedAttributesArr) {
			//sets all fields of entity xSingular.If xSingular does not exist nothing is done.
		...
		arrOfUserTypesForDropdown: function arrOfUserTypesForDropdown() {
			//returns an array of objects with keys value and text, mandatory for dropdowns.
		...
		userType: function userType(attributesElement) {
			//an element of attributesArr produced by csvStore.getAttributesArrNoId() or FL.dd.getArrayOfFields(entityName);//we retrieve all except name="id
			...
			var userType = null;
			if (attributesElement.type == "string") {
				//type is one of: string, integer, number, boolean, date, or json (Nico's field "M")
		...
		emptyRow: function emptyRow(xSingular) {
			//returns an object with keys for every attribute and empty values for each attribute
			...
			if (oEntity) {
				...
				if (newRow.id) newRow.id = oEntity.lastId + 1;
			} else {
		...
		emptyRowForArrOfTypes: function emptyRowForArrOfTypes(columnsArr) {
			//returns an object with keys for every attribute and empty values for each attribute
		...
		createEntityAndFields: function createEntityAndFields(entityName, entityDescription, fieldDefinitionArray) {
			//creates a dd entity with a set of fields
			...
			var xRet = false;
			if (FL.dd.createEntity(entityName, entityDescription)) {
		...
		relationsOf: function relationsOf(xSingular) {
			//returns an array with all relations of xSingular
		...
		removeRelations: function removeRelations(xSingular, sRightEntity) {
			//removes direct and reverse relations between the 2 entities
			...
			if (oDirectEntity && oReverseEntity) {
				...
				_.each(directRelationsArr, function (element, index) {
					if (element.rightEntity == sRightEntity) indexArr.push(index);
				});
				...
				_.each(reverseRelationsArr, function (element, index) {
					if (element.rightEntity == xSingular) indexArr.push(index);
				});
				_.each(indexArr, function (element) {
		...
		removeEntity: function removeEntity(xSingular) {
			//removes  entity xSingular in local dictionary
			...
			if (oEntity) {
				...
				for (var i = 0; i < xRelArr.length; i++) {
					...
					if (xRightEntity) FL.dd.removeRelations(xSingular, xRightEntity); //this removes direct and inverse relations
				};
		...
		getDictEntityBackup: function getDictEntityBackup(entityName) {
			//returns a copy of an whole entity (with all its internal objects)
		...
		reverseEnt: function reverseEnt() {
			var z = {};
			...
			_.each(oEntities, function (value, key) {
				//mounts a JSON whose keys are eCN -the loop has each entity
				if (key != "__Last" && key != "__LastRelation") {
					var fieldsObj = {};
					var thiz = this;
					_.each(value.attributes, function (element) {
						//for each entity mounts a JSON whose keys are fCN
						var fCN = value.L2C[element.name];
						element["parent"] = key;
						element["disp"] = function (x) {
							return "hello " + x;
						};
						element["setField"] = function (options) {
							//options:(name,description,label,type,typeUI,enumerable)
					...
					properties["getFieldCName"] = function (fieldName) {
						//if field name does not exist returns null
						var entityName = this.singular;
					...
					properties["addField"] = function (name, description, label, type, typeUI, arrEnumerable) {
						//if it exists update it otherwise a new is created
					...
					properties["getCName"] = function (fieldName) {
						//if field name does not exist returns null
			...
			z["getCName"] = function (entityName) {
				//if entity name does not exist returns null
				var oEntity = FL.dd.entities[entityName];
				if (FL.dd.entities[entityName]) return FL.dd.entities[entityName].csingular;
		...
		entityX: { //object model mockup
			...
			add: function add(entityName, optionsObj) {
			...
			getCName: function getCName(entityName) {
				var oEntity = FL.dd.entities[entityName];
				if (FL.dd.entities[entityName]) return FL.dd.entities[entityName].csingular;
				return null; //oEntity; //gives undefined
			},
