...
FL["grid"] = (function () {
	//name space FL.grid
	var prepareAttributesArr = function prepareAttributesArr(headerstring) {
		// var attributesArr = [];
	...
	var prepareOneEmptyRowArray = function prepareOneEmptyRowArray(arrOfColumns) {
		var emptyObj = {};
	...
	var injectId = function injectId(idTitle, arrOfColumns) {
		//injects a first id column into the array
	...
	var extractUniqueFromArray = function extractUniqueFromArray(arr) {
		...
		if (typeof emptyPair !== "undefined") {
			//the case where there is no emptyPair =>undefined
			empties = emptyPair[1];
		}
	...
	var dataRowAnalisys = function dataRowAnalisys(rows, percent) {
		...
		_.each(arrOfKeys, function (element, index) {
			...
			if (dataType == "string") {
				//possible datatypes:string, integer, number, boolean, date, or json
				...
				if (numberObj.number) {
					//this string is a number in string format
				...
				} else if (FL.common.is_dateArrInStringFormat(arrOfSampleRowValues)) {
					//values are in string format but are they a deguised date column ?
					obj[element] = { "fieldType": "date", "fieldTypeUI": "datetextbox", "numberFormat": null, enumerable: null, label: element };
				} else if (FL.common.is_enumerableArr(arrOfSampleRowValues, percent)) {
					//It is an enumerable we will prepare the enumerable content
					...
					//not a string number, note a string date and  not an enumerable
					...
					if (is_ColumnOfSubtype("email", uniqueObj.uniqueArr)) //checks that all sample objects are of subtype
						obj[element]["fieldTypeUI"] = "email";else if (is_ColumnOfSubtype("url", uniqueObj.uniqueArr)) obj[element]["fieldTypeUI"] = "url";else if (is_ColumnOfSubtype("phone", uniqueObj.uniqueArr)) obj[element]["fieldTypeUI"] = "phone";
				}
			} else {
				if (dataType == "number") {
					//possible datatypes:string, integer, number, boolean, date, or json
	...
	var translateToDDFormat = function translateToDDFormat(arrOfAttributes) {
		//translates arrOfAttributes format to dd format
	...
	var _adjustRowsToAttributes = function _adjustRowsToAttributes(rows, arrOfAttributes) {
		//creates the equivalent to a dd entry from a set of rows
		...
		_.each(arrOfAttributes, function (element, index) {
			...
			if (fieldType == "string" && fieldTypeUI == "combobox") {
				...
				// columnVector = _.pluck(rows,attrName);
				_.each(rows, function (rowElement, index) {
					if (typeof rowElement[attrName] != "date") {
	...
	var is_ColumnOfSubtype = function is_ColumnOfSubtype(subtype, sampleArr) {
		//scans all elements of sampleArr and returns true if all non empty elements belongs to subtype
		//example
	...
	var editGrid = function editGrid(entityName) {
		...
		var options = {
			...
			detailDropdown: {
				"userType": {
					...
					onSelect: function onSelect(objSelected, line) {
						...
						var enumStr = "";
						if (enumArr) enumStr = enumArr.join(",");
		...
		FL.common.editMasterDetail("B", " Define Table Dictionary", "_dictEditEntityTemplate", masterDetailItems, options, function (result) {
			if (result) {
				...
				_.each(attributesArrNoId, function (element, index) {
					//to retrieve the lines content from the interface
					var elObj = {};
					elObj["oldName"] = attributesArrNoId[index].name;
					elObj["name"] = masterDetailItems.detail[index].attribute.trim();
					if (elObj["name"] != attributesArrNoId[index].name) changedAttributesArr.push([attributesArrNoId[index].name, elObj["name"]]); //oldName,newName
					...
					if (elObj["type"] != attributesArrNoId[index].type) changedTypeArr.push([attributesArrNoId[index].name, attributesArrNoId[index].type, elObj["type"]]); //oldName,oldType,newType
	...
	var updateDictionaryAndTypesInServer = function updateDictionaryAndTypesInServer(entityName, newSingularName, description, newAttributesArr, changedAttributesArr, changedTypeArr) {
		...
		if (FL.API.nicoTestDuplicateIds(storedArr)) {
			//only works if no duplicate _id exist in csvStore
		...
		updateDictionaryAllAttributesPromise.done(function () {
			//this update local and server dictionary
			if (changedTypeArr.length > 0) {
				//there is type change(s)
	...
	var addImageToMandrillImageArr = function addImageToMandrillImageArr(name, srcContent) {
		//mandrillImagesArr is formed to FL.login.emailImagesArray
	...
	var elementTemplateHTML = function elementTemplateHTML(type, thiz) {
	...
	var buildSocialDivHTML = function buildSocialDivHTML(item) {
	...
	var appendTemplate = function appendTemplate(jsonObject, parentElement, centralWidth, percent, marginL, marginR, bgColor) {
    ...
    */
				...
				if (w > thiz.centralWidth) {
					//reduce size keeping ratio: (convenient for outlook)  width="400px" height="auto"
	...
	var getHTMLContent = function getHTMLContent(jsonTemplate) {
		// alert(jsonTemplate);
	...
	var convertsToArrOfObj = function convertsToArrOfObj(templateOptionsArr) {
	...
	var extractSenderObjFromModal = function extractSenderObjFromModal() {
		//extracts senderObj from _sendNewsletterTemplate
	...
	var newsletterEmissionUI = function newsletterEmissionUI(templateOptionsArr, entityName, grid, emailAttributeName) {
		//grid is the backgrid object necessary for select
		var def = $.Deferred();
		...
		var options = {
			...
			dropdown: {
				"_sendNewsletter_template": {
					arr: arrOfObj, //titles,
					"default": "No template",
					onSelect: function onSelect(objSelected) {
						// FL.common.printToConsole("Template choice was "+objSelected.text + " cId=" + objSelected.cId);
		...
		FL.common.editMasterDetail("B", " Send email/Newsletter", "_sendNewsletterTemplate", masterDetailItems, options, function (result) {
			if (result) {
				//user choosed button2 ==>Send Newsletter button
	...
	var getSelectedEmailArray = function getSelectedEmailArray(grid, emailAttributeName) {
		//TEM DE RECEBER A GRID !!!!
		var emailsArr = [];
	...
	var prepareNewsletterEmission = function prepareNewsletterEmission(entityName, grid, emailAttributeName) {
	...
	var getMailchimpHTML = function getMailchimpHTML(cId) {
		var def = $.Deferred();
		var arr = null;
		var fl = FL.fl;
		if (fl) {
	...
	var getMailchimpTemplates = function getMailchimpTemplates() {
		var def = $.Deferred();
		var arr = null;
		var fl = FL.fl;
		if (fl) {
	...
	var prepareNewsletterMCEmission = function prepareNewsletterMCEmission(entityName) {
		...
		getTemplatesPromise.done(function (arrOfObj) {
			...
			FL.common.editMasterDetail("B", " Send MC email/Newsletter", "_sendNewsletterTemplate", masterDetailItems, options, function (result) {
				if (result) {
					...
					else FL.common.makeModalInfo("Canceled !!! No template selected.");
				} else {
	...
	var checkDuplicateEmission = function checkDuplicateEmission(entityName, NName, toSend, senderObj) {
	...
	var getLookupInGrid = function getLookupInGrid(oLayout) {
		//
		var lookupObj = null;
		FL.dd.init_t(); //to init the temporary subsystem
		_.each(oLayout.format, function (element) {
			...
			if (field.typeUI == "lookupbox") lookupObj = field.specialTypeDef; //{eCN:<entity compressed name>, fCN:<field compressed name>}
		});
		return lookupObj;
	};
	var mountGridFromColumnsArr = function mountGridFromColumnsArr(columnsArr, eCN) {
		//
	...
	return {
		...
		createGrid: function createGrid() {
			//call with menu key "uri": "javascript:FL.grid.createGrid()"
			var masterDetailItems = {
			...
			FL.common.editMasterDetail("B", " Create Grid", "_createGridTemplate", masterDetailItems, options, function (result) {
				if (result) {
					...
					if (entityName === "" || headerString === "") FL.common.makeModalInfo("Empty sample or headerString => nothing to do");else {
		...
		adjustRowsToAttributes: function adjustRowsToAttributes(rows, arrOfAttributes) {
			_adjustRowsToAttributes(rows, arrOfAttributes);
		},
		verifyPapaFields: function verifyPapaFields(metaFieldsArr) {
			...
			_.each(fieldArray, function (element, index) {
				numberOfFields++;
				if (index == 0) {
					//check semicolumns anbd comma repetiions on firtst line
			...
			if (metaFieldsArr.aborted) ok = false; //one criteria to repeat
			...
			if (emptyFields > 0) {
				if (semi_colon_repetitions > numberOfFields) {
					//one heuristic criteria
		...
		validateCSV: function validateCSV(fileObj) {
			//this is called directly from template id="_importCSV" ->onchange="FL.grid.validateCSV(this.files)"
					...
					complete: function complete(results) {
						...
						if (resultObj.ok) {
							...
							Papa.parse(csvFile, {
								...
								complete: function complete(results) {
									data = results;
									if (results.data.length < rowsOnAttempt1) {
										//refuse because lines will be lost
		...
		importGrid: function importGrid() {
			//call with menu key "uri": "javascript:FL.grid.importGrid()" in Settings
			...
			FL.common.editMasterDetail("B", " Upload CSV", "_importCSV", masterDetailItems, options, function (result) {
				if (result) {
					...
					if (fileName === "")
						...
						FL.common.makeModalInfo("No CSV file selection - Nothing was done");else {
						//FL.ddcreateEntity()
		...
		csvToStore: function csvToStore(rows) {
			//rows is an array of JSON [{},{}...{}]; each JSON  has a key/value = attribute/content
		...
		insertDefaultGridMenu: function insertDefaultGridMenu(singular, plural) {
			// Adds a menu with title <plural> and content displayDefaultGrid(<singular>)
		...
		storeCurrentCSVToServerAndInsertMenu: function storeCurrentCSVToServerAndInsertMenu(entityName, insertMenu) {
			//(entityName,plural) Adds a menu with title <plural> and content displayDefaultGrid(<singular>)
			...
			var def = $.Deferred();
			var spinner = FL.common.loaderAnimationON("spinnerDiv");
			if (_.isUndefined(insertMenu) || insertMenu === null) insertMenu = true;
			var menuName = null;
			if (!entityName) {
		...
		updateCurrentCSVToServer: function updateCurrentCSVToServer(entityName) {
			//update all records of entityName Table existing in dictionary
			// entityName- Name of entity that will be stored
			// if entityName has spaces, they will be changed by "_"
			...
			if (FL.dd.isEntityInLocalDictionary(entityName)) {
				...
				removeAllPromise.done(function (data) {
					...
					var arrToSend = csvStore.extractFromCsvStore();
					if (arrToSend.length != data.count) alert("FL.grid.updateCurrentCSVToServer - ça c'est pas bon --->arrToSend.length=" + arrToSend.length + " vs data.count=" + data.count);
		...
		csvToGrid: function csvToGrid(csvFile) {
			//input is a JQuery object (a file representation). Ex var csvFile = $('input[type=file]');
		...
		removeLastRowIfIncomplete: function removeLastRowIfIncomplete(data) {
			...
			if (lastLineCols != totColsPerRow) data.data.splice(lastRowIndex, 1);
		},
		csvToGrid2: function csvToGrid2(csvFile, delimiter, encoding, entityName) {
			//input is a file object obtained from DOM	//http://papaparse.com/ 
		...
		displayDefaultGrid: function displayDefaultGrid(entityName) {
			//loads entity from server and display the grid with add,del,edit grid buttons at left and newsletter if a email field exist
			//condition to execute is: FL.API.serverCallBlocked = false;	
			entityName = entityName.replace(/_/g, " ");
			var spinner = FL.common.loaderAnimationON("spinnerDiv");

			var promiseUnblock = FL.API.checkServerCallBlocked().then(function () {
				...
				promise.done(function (data) {
					...
					if (lookupObj) {
						...
						openPromise.fail(function (err) {
							FL.common.printToConsole(">>>>>displayDefaultGrid openTable FAILURE <<<<<" + err, "API");def.reject(err);
						});
					} else {
		...
		displayDefaultGrid2: function displayDefaultGrid2(entityName) {
			//loads entity from server and display the grid with add,del,edit grid buttons at left and newsletter if a email field exist
			var def = $.Deferred();
			entityName = entityName.replace(/_/g, " ");
			// FL.common.spin(true);
			...
			var spinner = FL.common.loaderAnimationON("spinnerDiv");
			var promiseUnblock = FL.API.checkServerCallBlocked().then(function () {
				//this only occurs when checkServerCallBlocked is resolved
				var promise = FL.API.loadTable(entityName);
				promise.done(function (data) {
					csvStore.setEntityName(entityName); //stores <entityName> in csvStore object
					csvStore.store(data); //data is an array of objects [{},{},....{}] where id field is mandatory inside {}
					...
					FL.common.clearSpaceBelowMenus();
					$("#addGrid").show();$("#addGrid").html(" Add Row");$("#_editGrid").off("click");$("#_editGrid").click(function () {
						editGrid(entityName);
					});
					$("#_editGrid").show();
					if (FL.dd.isEntityWithTypeUI(entityName, "emailbox") || FL.dd.isEntityWithTypeUI(entityName, "email")) {
						//the newsletter option only appears to entities that have an email
						$("#_newsletter").off("click");
						...
						$("#_newsletter").show();$("#_newsletterMC").off("click");$("#_newsletterMC").click(function () {
							prepareNewsletterMCEmission(entityName);
						});
		...
		sendEmailTest: function sendEmailTest() {
			//sends a sample email with eMail/newsletter
