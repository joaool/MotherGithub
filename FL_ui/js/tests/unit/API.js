$(function () {
  ...
  asyncTest("FL.API.isUserExist(" + existingUser + ")", function () { //one test can have several assertions
    ...
    promise.done(function(exist){
        ...
        FL.common.printToConsole("test was done with success !");
        if(!exist){
  ...
  // FL.common.printToConsole(" ================================================= 3 prepareTrialApp ===================================================");
  // asyncTest("prepareTrialApp()", function () { //one test can have several assertions
  //   FL.common.printToConsole(" ================================================= 3.1 prepareTrialApp ===================================================");
  //   expect(1);
  //   FL.API.debug = false;
  //   var promise= FL.API.prepareTrialApp();
  //   promise.done(function(){
  //       // FL.common.printToConsole("==========================================================================================");
  //       // FL.common.printToConsole(JSON.stringify(FL.login.token));
  ...
  //             // FL.common.printToConsole("$$$$$ ok name does not exist and was registered $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$");
  ...
  //             FL.common.printToConsole("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  //             FL.common.printToConsole("Fail trying to registerTrialApp to user:" + newInexistingUser);
  //             FL.common.printToConsole("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
...
Test_createUserAndDefaultApp_connect()
  ...
  .then(Test_loadTable)
  .then(emptyTest)
  .fail(function(err){FL.common.printToConsole("BIG ERROR:"+err);})
  .always(function(){
    FL.API.debug = true; FL.common.printToConsole("************ tests are done !!! *************");
    FL.API.debug = false;
    // FL.API.disconnect();//to see results with rock mongo....
    FL.API.removeClientWithOneApp().then(function(){FL.API.debug = true; FL.common.printToConsole("Application was deleted !");});//the then is necessary to complete the promise 
});
...
function Test_createHistoMails_ifNotExisting(){
    ...
    asyncTest("--------->FL.API.createHistoMails_ifNotExisting()", function () { //one test can have several assertions
      FL.API.debug = false;
      // FL.common.printToConsole(" =================================================  createHistoMails_ifNotExisting =================================================");
      expect(1);
      ...
      promise.done(function(){//both cases of recently created or existing como here...
          FL.common.printToConsole("%%%----------------------------------------------------->createHistoMails_ifNotExisting OK");
      ...
      promise.fail(function(err){
          FL.common.printToConsole("%%%----------------------------------------------------->createHistoMails_ifNotExisting err="+err);
...
function Test_removeTable_existing(){
    ...
    asyncTest("--------->removeTable for existing table -->_histoMail" , function () { //one test can have several assertions
          // FL.common.printToConsole(" ================================================= 5.1 removeTable for existing table ===================================================");
          expect(1);
...
function Test_removeTable_unexisting(){
  ...
  asyncTest("--------->removeTable for unexisting table" , function () { //one test can have several assertions
    FL.API.debug = false;
    FL.common.printToConsole(" ================================================= 6.1 removeTable for unexisting table ===================================================");
    expect(1);
    var promise= FL.API.removeTable("blabla");
    promise.done(function(){
        // FL.common.printToConsole("Succeded removing table");
...
function Test_saveTable(){
  var def = $.Deferred();
  FL.common.printToConsole(" ================================================= 8 saveTable sales_rep with content  =================================================");
  ...
  asyncTest("--------->saveTable sales_rep with content [{'name':'Jojo','phone':'123'},{'name':'Anton','phone':'456'}]" , function () {
    FL.API.debug = false;
    FL.common.printToConsole(" ================================================= 8.1 saveTable sales_rep with content  =================================================");
    expect(1);
    var promise = FL.API.saveTable("sales_rep",records);
    // var promise=FL.API.isUserExist("customer1@xyz");
    promise.done(function(data){
        FL.common.printToConsole("Succeded saving table. returned:"+JSON.stringify(data));
        //data has the format:
...
function Test_addRecord(){
  var def = $.Deferred();
  FL.common.printToConsole(" ================================================= 9 addRecordsToTable to sales_rep   =================================================");
  FL.dd.createEntity("sales_rep","employee responsable for sales");
  FL.dd.addAttribute("sales_rep","name","sales_rep's name","Rep Name","string","textbox",null);
  FL.dd.addAttribute("sales_rep","phone","sales_rep's phone","Rep Phone","string","textbox",null);
  ...
  asyncTest("--------->addRecordsToTable to sales_rep with content [{'name':'George','phone':'789'},{'name':'Ringo','phone':'012'}]" , function () {
    FL.common.printToConsole(" ================================================= 9 addRecord to sales_rep  =================================================");
    expect(1);
    ...
    saveTablePromise.done(function(data){
        ...
        promise.done(function(data){
            FL.common.printToConsole("Succeded adding 2 records. returned:"+JSON.stringify(data));
...
function Test_loadTable(){
  ...
  asyncTest("--------->loadTable sales_rep" , function () { //one test can have several assertions
    FL.API.debug = false;
    FL.common.printToConsole(" ================================================= loadTable sales_rep ===================================================");
    expect(1);
    var promise= FL.API.loadTable("sales_rep");
    promise.done(function(data){
        FL.common.printToConsole("Succeded loading table sales_rep. returned:"+JSON.stringify(data));
...
function emptyTest(){
  ...
  // // FL.common.printToConsole(" ================================================= 5 FL.API.isUserExist =================================================");
  // // asyncTest("FL.API.customTable('shipment')", function () { //one test can have several assertions
  // //   FL.common.printToConsole(" ================================================= 5.1 FL.API.customTable({singular:'shipment'}); =================================================");
  // //   expect(5);
