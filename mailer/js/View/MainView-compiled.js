...
MailerTemplate.Views.MainView = Backbone.View.extend({
	...
	initialize: function initialize(expenses) {
	...
	OnPagePropertyChange: function OnPagePropertyChange(data) {
	...
	onModalOkClicked: function onModalOkClicked() {
		window.open("TemplatePreview.html", "_blank");
	},
	OnPreview: function OnPreview(evt) {
	...
	// 	//window.open("TemplatePreview.html","_blank");
	// },
	OnSaveTemplateBtnClick: function OnSaveTemplateBtnClick() {
		// var modelData = this.m_Editor.generatePlainHtml();
		var modelData = this.m_Editor.getModelData();
		var jsonData = this.m_jsonGenerator.GenerateJson(modelData);
	...
	OnLoadTemplateBtnClick: function OnLoadTemplateBtnClick() {
		// var temp = this;
		// $.ajax({
	...
	OnExitNoSave: function OnExitNoSave() {
		window.close();
	},
	...
	ShowProptertyPanel: function ShowProptertyPanel(data) {
		this.m_PropertyPanel.clear();
		this.m_PropertyPanel.show(data.type, data.model);
	},
	bindModelToPanel: function bindModelToPanel(model) {
		this.m_PropertyPanel.setModel(model);
	},
	isTemplateEmpty: function isTemplateEmpty(templateObj) {
	...
	storeTemplateUI: function storeTemplateUI(context, jsonData) {
	...
	storeTemplateForm: function storeTemplateForm(jsonData, arrOfObj) {
		//store jsonData on server eventually overlaping one of existing arrOfObj
		var jsonString = JSON.stringify(jsonData);
	...
	loadTemplateUI: function loadTemplateUI(context) {
		var spinner = FL.common.loaderAnimationON("spinnerDiv");
		var thiz = context;
		...
		promise.done(function (data) {
			...
			if (data.length === 0) {
				...
				selectedTemplatePromise.done(function (selectedTemplate) {
					...
					if (selectedTemplate) {
						//a specific template was selected
						...
						//no template was choosen (selectedTemplate=null)
						return;
					}
				});
				selectedTemplatePromise.fail(function (err) {
					console.log(">>>>>OnLoadTemplateBtnClick selectedTemplatePromise FAILURE <<<<<" + err);return def.reject(err);
				});
			}
		});
		promise.fail(function (err) {
	...
	loadTemplateForm: function loadTemplateForm(templateOptionsArr) {
		//returns a promise
		var def = $.Deferred();
	...
	convertsToArrOfObj: function convertsToArrOfObj(templateOptionsArr) {
	...
	convertsIdsToArrOfObj: function convertsIdsToArrOfObj(templateOptionsArr) {
		//receives [{"_id": "t 115"},{"_id": "t 116"}] and returns [{value:1,text: "t 115"},{value:2,text: "t 116"}]
		return _.map(templateOptionsArr, function (el, index) {
			return { "value": index + 1, "text": el._id };
		});
	}
});
...
//# sourceMappingURL=MainView-compiled.js.map