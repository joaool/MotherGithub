					...
					*/
					...
					setupEvents:function(){
						this.mouseDownInsideHandlerArea=On.pausable(Dom.byId(this.avatarId),"mousedown", Lang.hitch(this,function(e){
							console.log(" - resizeMoveArea7.setupEvents   <<"+this.current.label+
									" >> xxxxxxxxxxxxxxxxxxxxxxxxx mouseDownInsideHandlerArea EXIT ");
							this.activate();
						}));
						...
						this.mouseDownOutsideHandler=On.pausable(window,"mousedown", Lang.hitch(this,function(e){
							// console.log("resizeMoveArea7 mousedown in "+this.current.label);
							// if(!this.forceTerminationStatus) {
								console.log(" - resizeMoveArea7.setupEvents   "+this.current.label+
										"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  ResizeMoveArea mouseDownOutsideHandler EXIT "+
										"terminationStatus="+this.forceTerminationStatus);
								if(this.endCallback)
									this.endCallback();
					...
					toggleHandles: function(isHandles) {
						if (this.objResizeWidget) {
							...
							if(!this.handlesStatus) {
								console.log("------------------------------------------------------------------------------>"+
											"vai apagar os handles para "+this.avatarId);
					...
					activate:function(){
						...
						//if(!this.objResizeWidget){//ResizeWidget is created and its coordinator property is set to resizeMoveArea (this class)
							...
							this.partialResizeHandler=this.objResizeWidget.on("_onResizeComplete",Lang.hitch(this,function(e){//callback with an anonimous function
								...
								// console.log("ResizeMoveArea7.activate ---->@@@@@@@@-onResizeParcial-@@@@@@@ "+thiz.current.label+
								//		"---> catch _onResizeComplete EVENT W,H="+thiz.objResizeWidget.targetW+","+
								//		thiz.objResizeWidget.targetH);
								console.log("ResizeMoveArea7.activate ---->@@@@@@@@-onResizeParcial-@@@@@@@ "+thiz.current.label+
						...
						*/
						...
						On(this.objMove, "MoveStart", function (mover) { //return from from moveable when mouse is up
							...
							console.log("@@@@@@@@-onMoveStart-@@@@@@@ "+thiz.current.label+" move#"+thiz.moveCounter+" ---> catch onMoveStart EVENT x,y="+x+","+y);
						});				
						...
						On(this.objMove, "MoveStop", function (mover) { //return from from moveable when mouse is up
							...
							var x=thiz.position.x;
							var y=thiz.position.y;
							console.log("ResizeMoveArea7.activate MoveStop EVENT x="+x+" y="+y);
