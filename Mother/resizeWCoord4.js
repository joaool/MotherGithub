		...
		function(dojo,Declare,Dbg,Win,Lang,Event,Array,On,Mouse,Topic,Query,DomStyle,DomGeom,DomConstruct,DomClass,Dom,DomAttr,Exchanger,Evented){
			...
			//return Declare("resizeWidget", [Widget,Evented], {
				...
				constructor:function(args,thiz,oBoundaries){
					...
					this.oDbg=new Dbg();
					this.oDbg.setThis("resizeWCoord4");
					...
					if(this.oDbg.isDbg("constructor")) this.oDbg.display("*********************************** ENTRANCE WITH gridPattern="+this.gridPattern);
					...
					this.postCreate();
					//if(this.oDbg.isDbg("constructor")) this.oDbg.display("Leaving CONSTRUCTOR");
				},
				postCreate: function() {
					...
					if(this.oDbg.isDbg("postCreate")) this.oDbg.display("*********************************** ENTRANCE WITH gridPattern="+this.gridPattern);
					...
					this.mouseDownInsideHandler=On.pausable(this.targetNode,"mousedown", Lang.hitch(this,function(e){
						...
						if(this.resizeMoveStatus>0){
							if(this.oDbg.isDbg("postCreate")) this.oDbg.display("239 **************************   mouseDownInsideHandler ************************");
							if(this.oDbg.isDbg("postCreate")) this.oDbg.display("@@@-> this._mouseDown="+this._mouseDown+" isResize="+isResize);
					...
					if(this.oDbg.isDbg("postCreate")) this.oDbg.display("VAI CHAMAR this.createResizeHandles()");
					this.createResizeHandles();
					if(this.oDbg.isDbg("postCreate")) this.oDbg.display("CHAMOU this.createResizeHandles()");
				},
				_mouseDown: false,
				_lastDirection: "",
				/*
				...
				*/
				...
				//DomStyle.set(this.targetNode,"visibility","visible");
				//DomStyle.set(this.phantomNode,"visibility","hidden");
					var isResize=this.callerObj.resizeStatus;
					//this.callerObj.initStatus=false;
					if(this.oDbg.isDbg("_onMouseUp")) this.oDbg.display("resizeWCoord - User did 'MOUSE UP'. isResize="+isResize+" - gridPattern="+this.gridPattern+" this._mouseDown="+this._mouseDown);
					if(isResize){//only enters if not under moveCoord control
						if(this.oDbg.isDbg("_onMouseUp")) this.oDbg.display("isResize=TRUE !!!");
						var x = e.clientX; 
						var y = e.clientY; 
						...
						if (this._mouseDown) {
							...
							if(this.oDbg.isDbg("_onMouseUp")) this.oDbg.display("after onResizeComplete() !!!");
							if(this.oDbg.isDbg("_onMouseUp")) this.oDbg.display("this._mouseDown SET TO FALSE !!!!");
							if(this.oDbg.isDbg("_onMouseUp")) this.oDbg.display("-------------------------------------------------------------------");
							...
							if((x>=this.targetL) && (x<=this.targetL+this.targetW) && (y>=this.targetT) && (y<=this.targetT+this.targetH)){
								...
								if(this.oDbg.isDbg("_onMouseUp")) this.oDbg.display("MOUSE IS INSIDE FRAME !!!");
							};	
							this.lastX=x;	
							this.lastY=y;	
						...
						//this.endCallback(); //the way out !!!
							...
							if(this.oDbg.isDbg("_onMouseUp")) this.oDbg.display("369 @@@@@@@@@@@@@@@@@ upHandler and moveHandler REMOVED !!!");
							if(this.oDbg.isDbg("_onMouseUp")) this.oDbg.display("Next step will emit EVENT '_endResize' with inside="+this.inside);
							if(this.inside)
